import { type ProductNotFoundError } from '@errors/models/products/product-not-found.error'
import { ProductsNotFoundError } from '@errors/models/products/products-not-found.error'
...
export namespace CreateOrderUseCaseDTO {
  ...
  export type ResultFailure = Readonly<
    RepositoryError | GenerateIDError | ProductsNotFoundError | InvalidIDError | ProductNotFoundError
  >
...
> {
  ...
  protected async performOperation(parameters: CreateOrderUseCaseDTO.Parameters): CreateOrderUseCaseDTO.Result {
    ...
    if (foundProducts.length !== validatedProductIDs.length && notFoundProducts.length > 0) {
      return failure(new ProductsNotFoundError({ products: notFoundProducts }))
    }
