...
class Server {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.server = serve({
        fetch: app.fetch,
        port: productInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port ${productInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT}`
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port http://localhost:${productInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT}/docs`
      })
