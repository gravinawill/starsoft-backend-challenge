...
class Server {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.server = serve({
        fetch: app.fetch,
        port: productsInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port ${productsInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT}`
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port http://localhost:${productsInventoryServerENV.PRODUCT_INVENTORY_SERVER_PORT}/docs`
      })
