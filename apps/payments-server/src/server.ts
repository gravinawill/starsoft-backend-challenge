...
class Server {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.server = serve({
        fetch: app.fetch,
        port: paymentsServerENV.PAYMENTS_SERVER_PORT
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port ${paymentsServerENV.PAYMENTS_SERVER_PORT}`
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port http://localhost:${paymentsServerENV.PAYMENTS_SERVER_PORT}/docs`
      })
