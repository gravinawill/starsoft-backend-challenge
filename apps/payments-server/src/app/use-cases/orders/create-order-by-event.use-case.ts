...
> {
  ...
  protected async performOperation(
    ...
    const resultValidatePaymentMethod = Order.validatePaymentMethod({ paymentMethod: parameters.payload.paymentMethod })
    if (resultValidatePaymentMethod.isFailure()) return failure(resultValidatePaymentMethod.value)
    const { paymentMethodValidated } = resultValidatePaymentMethod.value
