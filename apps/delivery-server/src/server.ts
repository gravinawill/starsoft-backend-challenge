...
class Server {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.server = serve({
        fetch: app.fetch,
        port: deliveryServerENV.DELIVERY_SERVER_PORT
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port ${deliveryServerENV.DELIVERY_SERVER_PORT}`
      })
      this.logger.sendLogInfo({
        message: `Server started successfully on port http://localhost:${deliveryServerENV.DELIVERY_SERVER_PORT}/docs`
      })
